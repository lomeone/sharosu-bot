...
const gameReservation = (gameType: GAME_TYPE) => {
    return {
        getReservationInfo: (): {gameCount: number, reservation: Map<string, string>} => {
...
type Game = {
    ...
    reserve: (nicknames: Set<string>, time: string) => string,
    cancelReservation: (nicknames: Set<string>) => string,
    startGame: () => void,
    openReservationNextGame: () => {gameCount: number, reservation: Map<string, string>},
...
const monsterGame = (): Game => {
    ...
    return {
        ...
        getGameInformation: (): string => {
            const {gameCount, reservation} = monsterReservation.getReservationInfo();
            return getGameInformation(gameCount, reservation);
        },
        reserve: (nicknames: Set<string>, time: string): string => {
            const {gameCount, reservation} = monsterReservation.reserve(nicknames, time);
            return getGameInformation(gameCount, reservation);
        },
        cancelReservation: (nicknames: Set<string>): string => {
            const {gameCount, reservation} = monsterReservation.cancelReservation(nicknames);
            return getGameInformation(gameCount, reservation);
        },
...
const sitAndGoGame = (): Game => {
    ...
    return {
        ...
        getGameInformation: (): string => {
            const {gameCount, reservation} = sitAndGoReservation.getReservationInfo();
            return getGameInformation(gameCount, reservation);
        },
        reserve: (nicknames: Set<string>, time: string): string => {
            const {gameCount, reservation} = sitAndGoReservation.reserve(nicknames, time);
            return getGameInformation(gameCount, reservation);
        },
        cancelReservation: (nicknames: Set<string>): string => {
            const {gameCount, reservation} = sitAndGoReservation.cancelReservation(nicknames);
            return getGameInformation(gameCount, reservation);
        },
...
const weeklyTournamentGame = (): Game => {
    ...
    return {
        ...
        getGameInformation: (): string => {
            const {gameCount, reservation} = weeklyTournamentReservation.getReservationInfo();
            return getGameInformation(gameCount, reservation);
        },
        reserve: (nicknames: Set<string>, time: string): string => {
            const {gameCount, reservation} = weeklyTournamentReservation.reserve(nicknames, time);
            return getGameInformation(gameCount, reservation);
        },
        cancelReservation: (nicknames: Set<string>): string => {
            const {gameCount, reservation} = weeklyTournamentReservation.cancelReservation(nicknames);
            return getGameInformation(gameCount, reservation);
        },
...
const isNotStaff = (sender: string): boolean => {
    return !isStaff(sender);
};

const checkStaff = (sender: string) => {
    if (isNotStaff(sender)) {
        throw notStaffError();
    }
};

const addStaff = (nickname: string) => {
    staffList.add(nickname);
};

const removeStaff = (nickname: string) => {
                ...
                if (game !== undefined) {
                    if (msgTokenizer[1]) {
                        if (msgTokenizer[1] === "예약") {
                            ...
                            checkStaff(sender);
                            game.startGame();
                            replier.reply(game.gameType + "게임 예약이 마감되었습니다\n별도 예약없이 매장에 방문하시면 바로 게임을 즐기실 수 있어요");
                        } else {
